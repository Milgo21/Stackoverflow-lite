...
export class UserEffects{
  constructor(@Inject(Actions) private authService: AuthenticaionService, private action:Actions, private auth:AuthService){}
  ...
  //     exhaustMap(action=>{
  //       return this.authService.login(action.newUser).pipe(
  //         map(res=>{
  //           // this.auth.getRole(res.is_admin)
  //           this.auth.login()
  //           localStorage.setItem('token',res.token)
  //           return Signupactions.loginUserSuccess({res})}),
  //         catchError(error => of (Signupactions.loginFail({error: error.message})))
  //       )
  //     })
  //   )
  // })
